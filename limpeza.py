...
def parse_valor(v):
    ...
    Normaliza formatos:
      ...
      - Ambíguo: "1.500.000"   -> 1500.0
        NOTA: valor_original: 1.500.000 | sugestao: assumido 1500.00
    """
    if pd.isna(v):
        return None
    v = str(v).strip()
    ...
    if re.match(r'^\-?\d{1,3}(\.\d{3})*(,\d+)?$', v):
        ...
        # DECISAO: assumido 1500.00 — erro de milhar extra
...
df['nota'] = ''
df.loc[df['id'] == 13, 'nota'] = 'valor_original: 1.500.000 | sugestao: assumido 1500.00 (erro de digitacao de milhar)'
...
# ============================================================
def parse_data(d):
    """Tenta YYYY-MM-DD e DD/MM/YYYY. Datas impossíveis viram NaT."""
...
# DECISOES em datas inválidas:
# id=14: 32/01/2024 -> corrigido para 2024-01-31 (último dia válido de jan)
df.loc[df['id'] == 14, 'nota'] = 'data_original: 32/01/2024 | sugestao: corrigido para 2024-01-31'
df.loc[df['id'] == 14, 'data'] = pd.Timestamp('2024-01-31')

# id=15: 2024-13-01 -> corrigido para 2024-01-13 (inversão mês/dia)
df.loc[df['id'] == 15, 'nota'] = 'data_original: 2024-13-01 | sugestao: corrigido para 2024-01-13 (inversao mes/dia)'
...
if not negativos.empty:
    pendencias.append(f"  Valores negativos (verificar estorno) id(s): {negativos['id'].tolist()}")

if pendencias:
    print("PENDENCIAS PARA REVISAO MANUAL:")
    for p in pendencias:
        print(p)
else:
    print("OK - Nenhuma pendencia encontrada.")

# ============================================================
# EXPORTAR
# ============================================================
df.to_csv('vendas_limpo.csv', index=False)
print(f"Exportado: vendas_limpo.csv ({len(df)} registros)")
